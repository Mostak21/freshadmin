...
class CheckoutController extends Controller
{
    ...
    {
      $availavelItems = [];
      Session::forget('cartSelectedItem');
      if ($request->selectedItem != null){
          $carts = Cart::where('user_id', Auth::user()->id)->whereIn('id', $request->selectedItem)->get();
      }
      else{
      ...
      foreach ($carts as $key=> $cart){
          ...
          if ($stock && $stock->qty){
              array_unshift($availavelItems,$cart->id);
          }
          if ($stock && !$stock->qty || !$stock){
              flash(translate("Some of your items in cart are stock out"))->warning();
          }
      }
      if (!empty($availavelItems)){
          Session::put('cartSelectedItem', $availavelItems);
      }
      else{
          return back();
      }
...
//        if (Session::has('cart') && count(Session::get('cart')) > 0) {
    ...
    public function store_shipping_info(Request $request)
    {
        ...
        if ($request->session()->get('cartSelectedItem')!=null){
            $carts = Cart::where('user_id', Auth::user()->id)->whereIn('id', $request->session()->get('cartSelectedItem'))->get();
        }
        else{
            $carts = Cart::where('user_id', Auth::user()->id)->get();
        }
    ...
    public function store_delivery_info(Request $request)
    {
        ...
        if ($carts && count($carts) > 0) {
            ...
            $partial_payment = ceil($total*(10/100));
    ...
    public function apply_coupon_code(Request $request)
    {
        ...
        if ($request->session()->get('cartSelectedItem')!=null){
            $carts = Cart::where('user_id', Auth::user()->id)
                ...
                ->whereIn('id', $request->session()->get('cartSelectedItem'))
                ->get();
        }
        else{
            $carts = Cart::where('user_id', Auth::user()->id)
