...
if (!function_exists('filter_products')) {
    function filter_products($products)
    {
        ...
        if (get_setting('vendor_system_activation') == 1) {
            return $products->where('approved', '1')->where('published', '1')->where('auction_product', 0)->where(function ($p) use ($verified_sellers) {
...
if (!function_exists('renderStarRating')) {
    ...
    {
        $fullStar = "<i class = 'las ci-star-filled active' style='font-size:10px; letter-spacing: 2px;'></i>";
        $halfStar = "<i class = 'las ci-star-half active' style='font-size:10px; letter-spacing: 2px;'></i>";
        $emptyStar = "<i class = 'las ci-star' style='font-size:10px; letter-spacing: 2px;'></i>";
...
function getShippingCost($carts, $index,$shipping_method,$agent_costs)
{
    ...
    if (get_setting('shipping_type') == 'flat_rate') {
        ...
        if ($shipping_method=='delivery_agent'){
            $calculate_shipping = $agent_costs->cost;
        }
...
//Return Parent category tree of a product in array
if (!function_exists('category_tree')) {
    function category_tree($category_id){
        $categories = \App\Models\Category::findOrFail($category_id);
        for ($x = $categories->level; $x >= 0; $x--) {
            $item_category[$x] = $categories->name;
            $item_category_parent_id = $categories->parent_id;
            if ($x != 0){
                $categories = \App\Models\Category::findOrFail($item_category_parent_id);
            }
        }
...
if (!function_exists('category_bread_tree')) {
    function category_bread_tree($category_id){
        $categories = \App\Models\Category::findOrFail($category_id);
        for ($x = $categories->level; $x >= 0; $x--) {
            $item_category[$x][0] = $categories->name;
            $item_category[$x][1] = $categories->id;
			$item_category[$x][2] = $categories->auto_description;
            $item_category[$x][3] = $categories->auto_cat_description;
            $item_category_parent_id = $categories->parent_id;
            if ($x != 0){
                $categories = \App\Models\Category::findOrFail($item_category_parent_id);
            }
...
//automated description for product and category
if (!function_exists('autodiscription')) {
    function autodiscription($productname,$category,$brand)
    {



        if(isset($category[2][2])){
            $text= $category[2][2];
        }
        elseif (isset($category[1][2])) {
            $text= $category[1][2];
        }
        else
        $text = $category[0][2];
        ...
        $cat2=null;
        $cat3=null;
        if(isset($category[0][0]))
         $cat1= $category[0][0];
         if(isset($category[1][0]))
         $cat2= $category[1][0];
         if(isset($category[2][0]))
         $cat3= $category[2][0];

        eval("\$autodiscription = \"$text\";");
        echo $autodiscription. "\n";


    }
}

if (!function_exists('auto_cat_description')) {
    function auto_cat_description($category,$brand,$current_cat)
    {

        if( Route::currentRouteName()=="products.category")

        {
            if(isset($category[2][3])){
                $text= $category[2][3];
            }
            elseif (isset($category[1][3])) {
                $text= $category[1][3];
            }
            else
            $text = $category[0][3];
            ...
            if(isset($category[0][0]))
             ...
             if($brand){
                $brand_name=array();
                foreach(\App\Models\Brand::whereIn('id',$brand)->get() as $brand)
                {
                    array_push($brand_name,$brand->name);
                }
                $brands=array_unique($brand_name);
                $allbrands= implode(',',$brands);

             }
            eval("\$auto_cat_discription = \"$text\";");
            echo $auto_cat_discription. "\n";
...
if (!function_exists('updateCurrentPrice')) {
    function updateCurrentPrice($carts){
        foreach ($carts as $key => $cart) {
            ...
            if ($product){
                ...
                if ($productStock){
                    ...
                    $productPrice = $productStock->price;
                    $discount_applicable = false;
