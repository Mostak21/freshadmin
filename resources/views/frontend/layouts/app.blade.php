  ...
  //do something
        ...
        function removeFromCart(key){
            $.post('{{ route('cart.removeFromCart') }}', {
                ...
                RIT.plugins.notify('success', "Item has been removed from cart");
        ...
        function addToCompare(id){
            $.post('{{ route('compare.addToCompare') }}', {_token: RIT.data.csrf, id:id}, function(data){
                $('#compare').html(data);
                RIT.plugins.notify('success', "Item has been added to compare list");
        ...
        function addToWishList(id){
            @if (Auth::check() && (Auth::user()->user_type == 'customer' || Auth::user()->user_type == 'seller'))
                $.post('{{ route('wishlists.store') }}', {_token: RIT.data.csrf, id:id}, function(data){
                    if(data != 0){
                        $('#wishlist').html(data);
                        RIT.plugins.notify('success', "Item has been added to wishlist");
                    }
                    else{
                        RIT.plugins.notify('warning', "Please login first");
                    }
                });
            @else
                RIT.plugins.notify('warning', "Please login first");
        ...
        function addToCart(){
            if(checkAddToCartValidity()) {
                ...
                RIT.plugins.notify('warning', "Please choose all the options");
            }
        }
        ...
        function categoryhover(){
             if($('#hover-category-menu1').hasClass('d-none')){
                $("#hover-category-menu1").addClass('active').removeClass('d-none');

            }
            else {
                $("#hover-category-menu1").addClass('d-none').removeClass('active');

            }
        }

        function categoryhoverMouseover(x){
             if($('#hover-category-menu1').hasClass('d-none')){
                $("#hover-category-menu1").addClass('active').removeClass('d-none');
            }
        }
        function categoryhoverMouseout(x){
             if($('#hover-category-menu1').hasClass('active')){
                 $("#hover-category-menu1").addClass('d-none').removeClass('active');
            }
        }
        ...
        function dclicknav(){
            if($('#dnav').hasClass('d-none')){
                $("#dnav").addClass('active').removeClass('d-none');

            }
            else {
                $("#dnav").addClass('d-none').removeClass('active');

            }
        }

        function collapsecat(x){
             if($('#catcollapse'+x).hasClass('d-none')){
                $("#catcollapse"+x).addClass('active').removeClass('d-none');

            }
            else {
                $("#catcollapse"+x).addClass('d-none').removeClass('active');
				...
				function growDiv() {
			 if($('#grow').hasClass('d-none')){
                $("#grow").addClass('active').removeClass('d-none');

            }
            else {
                $("#grow").addClass('d-none').removeClass('active');
